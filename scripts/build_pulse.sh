...
# Install libpulse*.so to $PREBUILT_DIR/arm64-v8a/lib/ — note that
# install_lib puts files at $PREBUILT_DIR/arm64-v8a/<name>, not in a
# lib/ subdir, so we copy manually to keep the layout Wine's pkg-config
# .pc file expects (libdir=.../lib).
PULSE_LIB_DIR="$PREBUILT_DIR/arm64-v8a/lib"
mkdir -p "$PULSE_LIB_DIR"
cp -f "$PA_BUILD"/src/pulse/libpulse.so.0.* "$PULSE_LIB_DIR/libpulse.so.0" 2>/dev/null || true
cp -f "$PA_BUILD"/src/pulse/libpulse-simple.so.0.* "$PULSE_LIB_DIR/libpulse-simple.so.0" 2>/dev/null || true
cp -f "$PA_BUILD"/src/pulse/libpulsecommon-*.so "$PULSE_LIB_DIR/libpulsecommon.so" 2>/dev/null || true
# Strip symbols to shrink the .so files.
strip --strip-unneeded "$PULSE_LIB_DIR"/*.so* 2>/dev/null || true
...
mkdir -p "$PREBUILT_DIR/arm64-v8a/include/pulse" \
         "$PULSE_LIB_DIR/pkgconfig"
cp -f "$PA_SRC"/src/pulse/*.h "$PREBUILT_DIR/arm64-v8a/include/pulse/"
# Some headers are generated by meson in the build dir.
cp -f "$PA_BUILD"/src/pulse/*.h "$PREBUILT_DIR/arm64-v8a/include/pulse/" 2>/dev/null || true
cp -f "$PA_BUILD"/src/pulse/version.h "$PREBUILT_DIR/arm64-v8a/include/pulse/" 2>/dev/null || true
# .pc file for Wine's configure to find via pkg-config
